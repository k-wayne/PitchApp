...
def category(cat):
    cat = cat
    pitches = Pitch.query.filter_by(category=cat).order_by(Pitch.posted.desc()).all()
...
def new_pitch():
    ...
    if form.validate_on_submit():
        ...
        pitches = Pitch.query.filter_by(category=category).order_by(Pitch.posted.desc()).all()
        return render_template('category.html', pitches=pitches)
...
def new_comment(pitch_id):
    ...
    return render_template('comments.html', form=form, comments=all_comments, pitch=pitch)
